...
- **Rewards & Costs:**
  ...
  - If the robot attempts to **'Continue Task'** on a low battery, there is a 50% chance it succeeds - earning $20
  -  50% chance its battery dies mid-task, which costs $40 to recover the robot and reset the task.
...
3.  Describe the transition probabilities for the robot starting its _next_ task. For example, "If the state is 'Low Battery' and the action is 'Go Charge', the probability of the next state being 'High Battery' is X%."
...
This asks for the probability of what state the robot will be in for the _next_ task, given its current state ('Low Battery') and the action taken. This is written as `P(s' | s, a)`, where `s'` is the next state.
...
- **Case 2: Action is 'Continue Task'**
  ...
  - **Conclusion:** In _both_ possible outcomes (success or failure), the robot starts its next task in the 'High Battery' state.
...
V(s) = The value of current state s

R(s,a) = Reward(Immediate) recived after taking an action based on current staten s (environment & agent states)

\gamma_ = Discount factor (0 < γ < 1) Eagerness to recieve immediate short terms gains over long term gains 

If discount factor is 0, agent is only concerned with immediate rewards. If discount factor is 1, agent values future rewards just as much as immediate rewards.

\sum_ = The sum of all possible next states s'

P(s'|s) = Transition probability of going to state s' from state s

V(s') = Value of next state s' (reward)
